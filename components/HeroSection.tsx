...
export default function HeroSection() {
  ...
  return (
    ...
    >
            ...
            {units.map(({ value, label }, i) => (
              <div key={label} className="flex items-center">
                <div className="text-center px-5 md:px-8">
                  <span
                    className="block font-display italic text-dark font-light leading-none"
                ...
                {i < units.length - 1 && (
                  <span
                    className="text-gold-light font-light text-2xl select-none"
                    aria-hidden="true"
                  >
                    ·
                  </span>
      ...
      {/* Scroll cue */}
      <div
        className="absolute bottom-8 left-1/2 -translate-x-1/2 flex flex-col items-center gap-3 animate-bounce drop-shadow-[0_1px_4px_rgba(0,0,0,0.18)]"
        aria-hidden="true"
      >
        <span className="font-sans text-dark-soft text-[13px] whitespace-nowrap">
          {t("hero.scroll")}
        </span>
        <svg
