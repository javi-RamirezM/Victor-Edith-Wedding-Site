      ...
      <div className="max-w-lg mx-auto px-6 pb-2">
        <div className="bg-white border border-dark/[0.07] px-6 py-5">
          ...
          <ol className="space-y-0" aria-label={t('rsvp.timelineTitle')}>
            {[
              { day: t('rsvp.timelineFriday'), desc: t('rsvp.timelineFridayDesc') },
              { day: t('rsvp.timelineSaturday'), desc: t('rsvp.timelineSaturdayDesc') },
              { day: t('rsvp.timelineSunday'), desc: t('rsvp.timelineSundayDesc') },
            ].map(({ day, desc }, i, arr) => (
              <li key={day} className="flex items-start gap-4">
                <div className="flex flex-col items-center flex-shrink-0">
                  <div className="w-2 h-2 rounded-full bg-gold mt-1" aria-hidden="true" />
                  {i < arr.length - 1 && (
                    <div className="w-px flex-1 bg-gold/25 mt-1 min-h-[28px]" aria-hidden="true" />
                  )}
                </div>
                <div className="pb-4">
